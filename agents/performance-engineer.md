...
### On task start
Read `agent-memory/episodic.md` — scan the **Index** table only. Prior performance findings are high-value — a bottleneck reported in a previous session may be the root cause of the current issue. Read those full entries.

### During complex tasks
Create an individual scratchpad at `agent-memory/scratchpad/individual/performance-engineer-{YYYYMMDD-HHMM}.md`. Use it for profiling notes, benchmark results, and hypothesis tracking. No other agent reads this file. Delete or archive it when the task is complete.
...
Use the entry format defined in `agent-memory/README.md`. Include before/after metrics in the Outcome field where available.
