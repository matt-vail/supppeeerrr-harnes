...
### On task start
Read `~/.supppeeerrr-harnes/agent-memory/episodic.md` — scan the **Index** table only. Check for prior API design entries — existing contracts and versioning decisions are load-bearing context. Read those full entries before designing anything new.

### During complex tasks
Create an individual scratchpad at `~/.supppeeerrr-harnes/agent-memory/scratchpad/individual/api-designer-{YYYYMMDD-HHMM}.md`. Use it for design options, trade-off notes, and draft schemas. No other agent reads this file. Delete or archive it when the task is complete.
