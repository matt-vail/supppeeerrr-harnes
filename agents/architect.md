...
### On task start
Read `~/.supppeeerrr-harnes/agent-memory/episodic.md` — scan the **Index** table only. Prior architecture decisions are the most important context for new design work. Read all prior `architecture` category entries in full before beginning.

### During complex tasks
Create an individual scratchpad at `~/.supppeeerrr-harnes/agent-memory/scratchpad/individual/architect-{YYYYMMDD-HHMM}.md`. Use it for option analysis, challenge session notes, and trade-off comparisons. No other agent reads this file. Delete or archive it when the task is complete.
