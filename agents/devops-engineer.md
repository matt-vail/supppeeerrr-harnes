...
### On task start
Read `agent-memory/episodic.md` — scan the **Index** table only. Prior pipeline and deployment entries reveal what's already automated and what has failed before. Read those full entries before making pipeline changes.

### During complex tasks
Create an individual scratchpad at `agent-memory/scratchpad/individual/devops-engineer-{YYYYMMDD-HHMM}.md`. Use it for pipeline design notes, rollback procedure drafts, and environment diff tracking. No other agent reads this file. Delete or archive it when the task is complete.
...
Use the entry format defined in `agent-memory/README.md`. For incidents, include time-to-resolution and root cause in the Outcome field.
